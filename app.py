...
# Инициализация MEXC клиента (принудительно mainnet)
mexc_client = MexcFuturesClient(testnet=False)
...
def handle_entry_signal(data, symbol, direction):
    ...
    try:
        ...
        # Лимит на размер позиции (максимум 1.0 для безопасности)
        if qty > 1.0:
            return {"success": False, "error": f"Размер позиции {qty} превышает лимит 1.0"}
