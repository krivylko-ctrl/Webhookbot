...
def handle_trailing_signal(data, symbol, direction):
    ...
    try:
        from bybit_v5_fixed import get_position, client
        ...
        if (now - last_update_time) < 2.0:  # 2 сек антиспам
            return {"success": False, "error": "Trail update too frequent (spam protection)"}
        
        # 7) Минимальный шаг
        tick_size = 0.01  # базовый для ETHUSDT
        min_step = tick_size
        if last_stop is not None and abs(new_trail_stop - last_stop) < min_step:
            return {"success": False, "error": f"Step too small: {abs(new_trail_stop - last_stop)} < {min_step}"}
        
        # 8) Создаем/обновляем триггерный стоп-ордер (НЕ market)
        ...
        if result.get("retCode") == 0:
            ...
            return {
                "success": True, 
                "message": f"Trail stop updated: {symbol} @ {new_trail_stop}",
                "response": {
                    "symbol": symbol,
                    "side": pos_side,
            ...
            logger.error(f"❌ Failed to set trail stop: {error_msg}")
            return {"success": False, "error": f"Failed to place stop order: {error_msg}"}

    except Exception as e:
        logger.error(f"Trail signal error: {e}")
        return {"success": False, "error": f"Trail processing error: {str(e)}"}
