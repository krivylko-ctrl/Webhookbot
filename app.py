...
def handle_exit_signal(data, symbol, direction):
    """Обработка сигнала выхода из позиции - НЕУБИВАЕМОЕ ЗАКРЫТИЕ"""
    try:
        logger.info(f"✅ Выполнение сигнала V5: exit {symbol}")
        
        # Инициализация клиента V5
        client = initialize_v5_client()
        if not client:
            return {"success": False, "error": "V5 Client не инициализирован"}
        
        # Нормализация символа - убираем .P если есть
        if symbol.endswith(".P"):
            symbol = symbol[:-2]
            logger.info(f"Нормализация символа: {symbol}.P → {symbol}")
        
        # Неубиваемое закрытие через market + reduceOnly + IOC
        # ИГНОРИРУЕМ qty из webhook - берем фактический размер позиции
        response = client.close_position_market_hard(symbol, prefer_cancel_all=True)
        
        if response.get("status") == "success":
            trade_stats["active_positions"] = max(0, trade_stats.get("active_positions", 1) - 1)
            logger.info(f"🔻 Позиция {symbol} закрыта неубиваемым методом")
            return {"success": True, "message": response.get("message"), "response": response}
        elif response.get("status") == "info":
            # Позиция уже закрыта или размер слишком мал
            logger.info(f"ℹ️ {response.get('message')}")
            return {"success": True, "message": response.get("message"), "response": response}
        else:
            return {"success": False, "error": response.get("message")}

    except Exception as e:
        logger.error(f"Ошибка обработки сигнала exit: {e}", exc_info=True)
