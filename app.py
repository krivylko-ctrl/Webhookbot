...
# Инициализация Bybit клиента (PRODUCTION MAINNET)
bybit_client = BybitFuturesClient(testnet=False)
...
def test_connection():
    """Тестирование соединения с Bybit PRODUCTION API"""
    try:
        result = bybit_client.test_connection()
        if "error" in result:
            add_log("ERROR", f"Ошибка подключения к Bybit PRODUCTION: {result['error']}")
            return jsonify({"status": "error", "message": result["error"]}), 500
        elif result.get("status") == "connected":
            add_log("SUCCESS", "Подключение к Bybit PRODUCTION API работает")
...
def handle_entry_signal(data, symbol, direction):
    """Обработка сигнала входа в позицию - PRODUCTION MAINNET"""
    try:
        ...
        logger.info(f"PRODUCTION ВХОД В ПОЗИЦИЮ: {symbol} {direction.upper()}")
        ...
        return {"success": True, "message": f"PRODUCTION позиция {direction} открыта", "response": response}

    except Exception as e:
        return {"success": False, "error": f"Ошибка входа: {str(e)}"}

def handle_exit_signal(data, symbol, direction):
    """Обработка сигнала выхода из позиции - PRODUCTION MAINNET"""
    try:
        logger.info(f"PRODUCTION ВЫХОД ИЗ ПОЗИЦИИ: {symbol} {direction.upper()}")
        ...
        return {"success": True, "message": f"PRODUCTION позиция {direction} закрыта", "response": response}

    except Exception as e:
        return {"success": False, "error": f"Ошибка выхода: {str(e)}"}

def handle_trailing_signal(data, symbol, direction):
    """Обработка сигнала обновления трейлинг стопа - PRODUCTION MAINNET"""
    try:
        ...
        logger.info(f"PRODUCTION ОБНОВЛЕНИЕ ТРЕЙЛИНГ СТОПА: {symbol} {direction.upper()}, новый стоп: {new_stop}")
        ...
        return {"success": True, "message": f"PRODUCTION трейлинг стоп обновлен", "response": response}

    except Exception as e:
        return {"success": False, "error": f"Ошибка обновления стопа: {str(e)}"}

def handle_manual_close(data, symbol, direction):
    """Обработка ручного закрытия позиции - PRODUCTION MAINNET"""
    try:
        logger.info(f"PRODUCTION РУЧНОЕ ЗАКРЫТИЕ: {symbol} {direction.upper()}")
        ...
        trade_stats["active_positions"] = max(0, trade_stats.get("active_positions", 0) - 1)
        return {"success": True, "message": f"PRODUCTION позиция {direction} закрыта вручную", "response": response}
...
if __name__ == '__main__':
    logger.info("Запуск PRODUCTION торгового бота...")
    add_log("INFO", "PRODUCTION торговый бот запущен")
