...
class BybitFuturesClient:
    def __init__(self, testnet=True):
        """
        Инициализация Bybit testnet клиента через CCXT
        ТОЛЬКО TESTNET - БЕЗ СИМУЛЯТОРА
        """
        self.testnet = True  # Принудительно только testnet
        
        # Получение API ключей из переменных окружения
        self.api_key = os.environ.get('BYBIT_API_KEY')
        self.api_secret = os.environ.get('BYBIT_API_SECRET')
        
        logger.info("Bybit testnet API инициализация через CCXT:")
        ...
        logger.info(f"  Режим: TESTNET ТОЛЬКО")
        ...
        # Инициализация CCXT клиента только для testnet
        try:
            if self.api_key and self.api_secret:
                self.exchange = ccxt.bybit({
                    'apiKey': self.api_key,
                    'secret': self.api_secret,
                    'sandbox': True,  # Принудительно testnet
            ...
            logger.info(f"  Base URL: https://api-testnet.bybit.com")
            logger.info(f"  API Version: V5")
            logger.info(f"  RecvWindow: 20000ms")
            logger.info("✅ ГОТОВ К ТОРГОВЛЕ: Bybit testnet API активен через CCXT")
    ...
    def test_connection(self) -> Dict[str, Any]:
        """Тестирование соединения с Bybit testnet"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
            
            balance = self.exchange.fetch_balance()
            logger.info("✅ Подключение к Bybit testnet API успешно")
            
            return {
                "status": "connected",
                "message": "Подключение к Bybit testnet API работает",
                "testnet": True,
                "balance": balance.get('USDT', {}).get('total', 0)
            }
            
        except Exception as e:
            logger.error(f"❌ Ошибка подключения к Bybit testnet API: {e}")
            return {
                "status": "error",
                "message": f"Ошибка подключения к Bybit testnet: {str(e)}",
                "testnet": True
            }
    ...
    def place_order(self, symbol: str, direction: str, quantity: float, 
        ...
        """Размещение ордера в Bybit testnet - ТОЛЬКО РЕАЛЬНЫЙ API"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
            ...
            logger.info(f"✅ Ордер размещен: {order.get('id')} - {direction} {quantity} {symbol}")
            ...
            logger.error(f"❌ Ошибка размещения ордера в Bybit testnet: {e}")
            return {
    ...
    def _place_stop_orders_ccxt(self, symbol: str, direction: str, quantity: float, 
        ...
        """Размещение стоп-ордеров через CCXT"""
        try:
            ...
            if stop_loss:
                ...
                logger.info(f"✅ Stop Loss установлен: {stop_loss} (ID: {sl_order.get('id')})")
            ...
            if take_profit:
                ...
                logger.info(f"✅ Take Profit установлен: {take_profit} (ID: {tp_order.get('id')})")
    ...
    def open_position(self, symbol: str, direction: str, quantity: float, 
                     entry_price: Optional[float] = None, stop_loss: Optional[float] = None, 
                     take_profit: Optional[float] = None, leverage: int = 1) -> Dict[str, Any]:
        """Открытие позиции - ТОЛЬКО РЕАЛЬНЫЙ API"""
    ...
    def close_position(self, symbol: str, direction: str = None) -> Dict[str, Any]:
        """Закрытие позиции - ТОЛЬКО РЕАЛЬНЫЙ API"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
    ...
    def update_stop_loss(self, symbol: str, direction: str, stop_price: float, trail_amount: float = None) -> Dict[str, Any]:
        """Обновление стоп-лосс - ТОЛЬКО РЕАЛЬНЫЙ API"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
    ...
    def get_account_info(self) -> Dict[str, Any]:
        """Получение информации об аккаунте - ТОЛЬКО РЕАЛЬНЫЙ API"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
            ...
            return {"error": str(e), "testnet": True}

    def get_positions(self, symbol: str = None) -> Dict[str, Any]:
        """Получение активных позиций - ТОЛЬКО РЕАЛЬНЫЙ API"""
        try:
            if not self.exchange:
                raise Exception("Exchange не инициализирован")
