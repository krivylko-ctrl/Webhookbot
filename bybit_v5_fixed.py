...
class BybitV5API:
    ...
    def get_position(self, symbol: str, category: str = "linear") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.make_request("GET", "/v5/position/list", params)
            
            if response.get("retCode") == 0:
                positions = response.get("result", {}).get("list", [])
                for pos in positions:
                    size = float(pos.get("size", "0"))
                    if size > 0:  # Активная позиция
                        return {
                            "symbol": pos.get("symbol"),
                            "side": pos.get("side", "").lower().replace("buy", "long").replace("sell", "short"),
                            "size": str(size),
    ...
    def get_tickers(self, category: str = "linear", symbol: str = None):
        """Получение тикеров"""
        params = {"category": category}
        if symbol:
            params["symbol"] = symbol
        return self.make_request("GET", "/v5/market/tickers", params)

    def place_order(self, **kwargs):
        """Размещение ордера"""
        return self.make_request("POST", "/v5/order/create", kwargs)

    def cancel_all_orders(self, **kwargs):
        """Отмена всех ордеров"""
        return self.make_request("POST", "/v5/order/cancel-all", kwargs)
...
def get_position(symbol: str):
    """Функция-обертка для получения позиции"""
    global v5_client
    if v5_client:
        return v5_client.get_position(symbol)
    return None
